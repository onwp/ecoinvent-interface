...
function getSecretsDir() {
    ...
    if (process.env.NODE_ENV !== 'test' && !fs__namespace.existsSync(secretsDir)) {
        try {
...
class Catalogue {
    ...
    constructor(filepath) {
        ...
        if (!fs__namespace.existsSync(this._filepath)) {
            this._write({});
        }
    ...
    _load() {
        try {
            const content = fs__namespace.readFileSync(this._filepath, 'utf8');
    ...
    _write(data) {
        try {
            fs__namespace.writeFileSync(this._filepath, JSON.stringify(data, null, 2), 'utf8');
        }
        catch (error) {
...
class BrowserCatalogue {
    ...
    async load() {
        try {
            const data = await idbKeyval.get('ecoinvent-catalogue');
            if (data) {
                this._data = data;
            }
        }
    ...
    async save() {
        try {
            await idbKeyval.set('ecoinvent-catalogue', this._data);
        }
        catch (error) {
...
class ProcessMapping {
    ...
    findClosestMatch(localProcess, remoteProcesses, threshold = 5) {
        ...
        for (const remoteProcess of remoteProcesses) {
            ...
            const dist = fastestLevenshtein.distance(localString, remoteString);
            if (dist <= threshold) {
